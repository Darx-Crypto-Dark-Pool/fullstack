...
export default function TradeSidebar({ token }: { token: string }) {
  ...
  const [showDemoPrivateKey, setShowDemoPrivateKey] = React.useState(false)
  ...
  const isDemoKeyEnabled = process.env.NEXT_PUBLIC_ENABLE_DEMO_KEY === 'true' && Boolean(demoPrivateKey)
  ...
  const handleCopyDemoKey = async () => {
    ...
    try {
      await navigator.clipboard.writeText(demoPrivateKey)
      ...
      {isDemoKeyEnabled && (
        <div className="rounded border border-yellow-500/30 bg-yellow-500/10 p-3 space-y-2">
          <p className="text-xs font-medium text-yellow-700 dark:text-yellow-300">
            Demo wallet key (hackathon/testnet only)
          </p>
          <p className="text-[11px] text-muted-foreground">
            Do not use this on mainnet or with real funds. Anyone who can see this UI can import the demo wallet.
          </p>
          <p className="break-all rounded bg-background/70 p-2 text-[11px]">
            {showDemoPrivateKey ? demoPrivateKey : '••••••••••••••••••••••••••••••••'}
          </p>
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="secondary"
              size="sm"
              onClick={() => setShowDemoPrivateKey((v) => !v)}
            >
              {showDemoPrivateKey ? 'Hide Key' : 'Reveal Key'}
            </Button>
            <Button
              type="button"
              variant="outline"
              ...
              disabled={!showDemoPrivateKey}
            >
              {copyStatus === 'copied' ? 'Copied' : copyStatus === 'failed' ? 'Copy Failed' : 'Copy Key'}
            </Button>
          </div>
        </div>
      )}
